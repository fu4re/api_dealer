...
const getSlideDate = ({
  ...
  const hasShown = child.classList.contains('-shown')
  const hasShownAnimated = child.classList.contains('-shownAnimated') // check -shownAnimated for Safari bug
  const isActiveSlide = hasShown || hasShownAnimated
